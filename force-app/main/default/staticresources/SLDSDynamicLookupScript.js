 ...
 */
function keyPressedOnLookup(componentId, objectName, fieldNames, fieldsPattern, photo, objectPluralName, remoteMethod, jsfunction, additionalFilter, spinner){
    clearTimeout(typingTimer);
    var selector = '#'+componentId;
    if (document.querySelector(selector+" #lookup").value) {
        typingTimer = setTimeout(startSearch(componentId, objectName, fieldNames, fieldsPattern, photo, objectPluralName, remoteMethod, jsfunction, additionalFilter, spinner), doneTypingInterval);
    }else{
 ...
 */
function startSearch(componentId, objectName, fieldNames, fieldsPattern, photo, objectPluralName, remoteMethod, jsfunction, additionalFilter, spinner) {
    var selector = '#'+componentId;
        ...
        function(result, event){
            if (event.status) {
                ...
                for(var i = 0; i < result.length ; i++){
                    var record = result[i];
                    recordLst.appendChild(createRecordLi(componentId,record,objectName, spinner));
                }
 ...
 * @return li
 */
function createRecordLi(componentId,record,objectName,spinner){
    ...
    li.setAttribute("onclick", "select('"+componentId+"', '"+displayName+"', '"+photoUrl+"', '"+id+"', '"+jsfunction+"', '"+objectName.toLowerCase()+"', "+ spinner + ")");
 ...
 */
function select(componentId, name,photoUrl,id, javascriptMethod, objectName, spinner){
    var selector = '#'+componentId;
    if (spinner) {
        showSpinner();
    }
 ...
 * @return {[type]} [description]
 */
function removeRecord(componentId, javascriptMethod, spinner){
